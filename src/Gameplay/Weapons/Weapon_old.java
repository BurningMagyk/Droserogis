...
public abstract class Weapon_old extends Item
{
    ...
    Weapon_old(float xPos, float yPos, float width, float height, float mass, String[] spritePaths)
    {
    ...
    public Weapon_old equip(Actor actor)
    {
    ...
    public Weapon_old unequip(float theta, Vec2 posOffset)
    {
        actor = null;
        ballistic = true;
    ...
    interface Operation
    {
        ...
        void apply(Weapon_old _this, Item other);
    ...
    abstract class Melee implements Operation
    {
        ...
        public void apply(Weapon_old _this, Item other)
        {
            ...
            if (other instanceof Weapon_old && !((Weapon_old) other).isBallistic())
            {
                if (!isIntersect(getShapeCorners(),
                        ((Weapon_old) other).getShapeCorners())) return;
                target = ((Weapon_old) other).actor;
            }
    ...
    abstract class Rush implements Operation
    {
        ...
        public void apply(Weapon_old _this, Item other)
        {
            ...
            //for (Weapon_old weapon : actor.weapons) { if (other == weapon) return; }
            if (other instanceof Weapon_old && !((Weapon_old) other).isOperating()) return;
    ...
    abstract boolean clash(Weapon_old otherWeapon, Operation otherOp, GradeEnum damage);
    abstract Vec2 getMomentum(Operation operation, DirEnum dir, Weapon_old other);
    ...
    protected void applyInflictions()
    {
        ...
        if (actor != null && this instanceof Natural)
        {
            for (int i = 0; i < inflictions.size(); i++)
            {
                ...
                if (inf.getDamage().ordinal() != 0 && actor.hasInfliction(inf))
                    inf.cancelDamage();
            }
        }
